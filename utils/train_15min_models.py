"""
Train 15-Minute Bitcoin Prediction Models
==========================================
Trains XGBoost models for 15-minute predictions (15m, 30m, 2h)
Uses Binance 15-minute data (60-90 days, ~5760-8640 samples)

Output:
- models/saved_models/15min/xgboost_15m.json
- models/saved_models/15min/xgboost_30m.json
- models/saved_models/15min/xgboost_2h.json
- models/saved_models/15min/scaler_15min.pkl
- models/saved_models/15min/feature_cols_15min.pkl
"""
...
def prepare_data_for_horizon(df, horizon_periods=1):
    """Prepare data for specific prediction horizon (in 15-min periods)"""
    df[f'future_return_{horizon_periods}p'] = df['close'].pct_change(horizon_periods).shift(-horizon_periods)
    df['current_price'] = df['close']

    df_clean = df.dropna(subset=[f'future_return_{horizon_periods}p']).copy()
    ...
    y = df_clean[f'future_return_{horizon_periods}p'].values
...
def main():
    ...
    print("\n" + "="*70)
    print("  15-MINUTE MODEL TRAINING PIPELINE")
    print("="*70)

    models_dir = Path('models/saved_models/15min')
    models_dir.mkdir(parents=True, exist_ok=True)

    print("\n[STEP 1] Loading cached Binance 15-minute data...")
    print("  Note: Using TRUE 15-minute candles from Binance")
    print("        Expected: ~5760 candles (60 days × 96 candles/day)")

    data_path = Path('data/raw/btc_binance_60d_15min.csv')
    ...
    print(f"✓ Loaded {len(df)} 15-minute candles from {data_path}")
    ...
    # 15-minute predictions: 1 period = 15 min, 2 periods = 30 min, 8 periods = 2 hours
    # Avoids overlap with hourly models (1h, 6h, 24h)
    horizons = [1, 2, 8]  # 15 min, 30 min, 2 hours ahead
    horizon_names = ['15m', '30m', '2h']
    all_metrics = []

    print(f"\n  Prediction horizons:")
    print(f"    - 1 period = 15 minutes ahead")
    print(f"    - 2 periods = 30 minutes ahead")
    print(f"    - 8 periods = 2 hours ahead")

    for horizon, name in zip(horizons, horizon_names):
        print(f"\n[STEP 3.{horizon}] Training {name} model...")
    ...
    scaler_path = models_dir / 'scaler_15min.pkl'
    feature_cols_path = models_dir / 'feature_cols_15min.pkl'
    ...
    metrics_df.to_csv(results_dir / '15min_models_metrics.csv', index=False)

    print(f"\n✓ Training complete!")
    print(f"✓ Models saved to: {models_dir}")
    print(f"✓ Metrics saved to: results/15min_models_metrics.csv")
    print("="*70 + "\n")
